...
function updateTransactions() {
  ...
  ui.alert("✅ Done!",
    ...
    (dashSheet ? "✔ Dashboard updated — all charts and numbers are current" : "⚠️ No Dashboard found — run Setup Dashboard first"),
    ui.ButtonSet.OK);
}
...
function setupDashboard() {
  ...
  // ── Right side: Top Payees QUERY (FIXED) ──────────────────────────
  // Added LABEL clause to prevent "sum" from appearing as a data value.
  // QUERY with headers=0 sometimes outputs aggregate function names ("sum")
  // when no LABEL is specified. Explicit LABEL fixes this.
  var payeeQuery =
    '=IFERROR(QUERY(\'Raw Data\'!A:G,' +
    '"SELECT C, SUM(F) ' +
    'WHERE G=\'"&' + SEL + '&"\' AND C <> \'\' AND F ' + expF +
    ' GROUP BY C ORDER BY SUM(F) ' + expSort + ' LIMIT 10 ' +
    'LABEL C \'Payee\', SUM(F) \'Amount\'"' +
  ...
  // ── Row 23: Monthly totals QUERY (excludes transfers) ─────────────────
  // Excludes: Income, Savings, Starting Balances (non-expenses)
  var monthlyQuery =
    '=IFERROR(QUERY(\'Raw Data\'!A:G,' +
    '"SELECT G, SUM(F) ' +
    'WHERE G MATCHES \'20[0-9][0-9]-[0-9][0-9]\' AND ' +
    'E<>\'Income\' AND E<>\'Savings\' AND E<>\'Starting Balances\' AND E<>\'\' ' +
    'GROUP BY G ORDER BY G DESC ' +
    'LABEL G \'Month\', SUM(F) \'Total Spent\'"' +
    ',0),{"Month","Total Spent"})';
  dashSheet.getRange(R.monthData, 1).setFormula(monthlyQuery);
  dashSheet.getRange(R.monthData, 2, 12, 1)
    .setNumberFormat('"$"#,##0.00;"$"#,##0.00');

  // ── Total Income column next to monthly spending (FIXED v3) ───────────
  // Uses QUERY with INDEX to extract just the income column.
  // Filters by Category="Income" to exclude transfers between accounts.
  // QUERY must SELECT both G and SUM(F) when GROUP BY G is used (SQL requirement).
  // INDEX extracts column 2 (income) so we don't duplicate the month column.
  var incomeQuery =
    '=IFERROR(INDEX(QUERY(\'Raw Data\'!A:G,' +
    '"SELECT G, SUM(F) ' +
    'WHERE G MATCHES \'20[0-9][0-9]-[0-9][0-9]\' AND E = \'Income\' ' +
    'GROUP BY G ORDER BY G DESC ' +
    'LABEL SUM(F) \'Total Income\'"' +
    ',0),,2),{"Total Income"})';
  dashSheet.getRange(R.monthData, 3).setFormula(incomeQuery);
  dashSheet.getRange(R.monthData + 1, 3, 12, 1).setNumberFormat('"$"#,##0.00');
...
function buildChartData_(dashSheet, R, expF, incF) {
  ...
  // Chart data: Reference the main Monthly table (A23:C) in reverse order (ASC)
  // This eliminates duplicate queries and ensures perfect alignment!
  var monthDataRow = R.monthData + 1; // First data row of monthly table (row 24)

  // Take first 6 months from main table, reverse them to ASC order for chronological chart
  dashSheet.getRange(dat, col).setFormula(
    '=INDEX(SORT(A' + monthDataRow + ':A' + (monthDataRow + 5) + ',1,TRUE),,1)'
  );
  dashSheet.getRange(dat, col + 1).setFormula(
    '=ARRAYFORMULA(ABS(INDEX(SORT(A' + monthDataRow + ':C' + (monthDataRow + 5) + ',1,TRUE),,2)))'
  );
  dashSheet.getRange(dat, col + 2).setFormula(
    '=INDEX(SORT(A' + monthDataRow + ':C' + (monthDataRow + 5) + ',1,TRUE),,3)'
  );

  dashSheet.getRange(dat, col + 1, 6, 2).setNumberFormat('"$"#,##0.00;"$"#,##0.00');

  // Hide columns J, K, L (chart helper data - users don't need to see this)
  dashSheet.hideColumns(col, 3); // Hide 3 columns starting from col J (column 10)
}
...
function buildCharts_(dashSheet, R) {
  ...
  // Sorted ASC (oldest to newest) for better chronological visualization.
